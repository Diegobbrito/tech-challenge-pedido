...
public class PedidoAdapter {
    ...
    public static PedidoResponse toResponse(Pedido pedido, PagamentoDtoResponse pagamento) {
        final var response = new PedidoResponse(pedido.getId(), formatarParaReal(pedido.getValor()), pedido.getStatus().getTipo());
    ...
    private static List<ProdutoSelecionado> getProdutosSelecionados(PedidoRequest request, List<Produto> produtos) {
        ...
        return produtos.stream().map(p -> new ProdutoSelecionado(p.getId(), produtoQuantidade.get(p.getId()))).collect(Collectors.toList());
    }
